...
var dataX = [],
    ...
    trainLen = 100400;
var nextFireEn = 0;
var model; // model of the tensorflow
var trainQ = 0; // get data or try to predict
...
class gameTrain extends Phaser.State {
    ...
    moveEnemy() {
        var move = this.predictMove();

        // move 25px to left or right depending on pred
        this.game.physics.arcade.moveToXY(this.player, this.player.x + (25 * move), 768, 500);
    }
    ...
    predictMove() {
        dataX = [this.player.x, this.bulletsEnemy.x];

        var pred = model.predict(tf.tensor([dataX]));
        console.log(pred);

        var nextMove = tf.argMax(pred, 1).dataSync();

        return ((nextMove === 0) ? nextMove - 1 : nextMove);
    }
}
